...
class TestAsyncConvertRun:
    """Tests for AsyncConvert.run() asynchronous conversion."""

    async def test_async_convert_with_url(
        self, async_client: AsyncClient, mock_api: respx.MockRouter
    ) -> None:
        """Async convert document from URL."""
        mock_api.post("/api/convert").mock(
            return_value=httpx.Response(
                200,
                json={
        ...
        result = await async_client.convert.run(
            url="https://example.com/invoice.pdf",
            document_type_code="invoice",
        )
    ...
    async def test_async_convert_with_bytes(
        self, async_client: AsyncClient, mock_api: respx.MockRouter
    ) -> None:
        """Async convert document from bytes."""
        mock_api.post("/api/convert").mock(
            return_value=httpx.Response(
                200,
                json={"data": {"field": "value"}},
    ...
    async def test_async_convert_with_metadata(
        self, async_client: AsyncClient, mock_api: respx.MockRouter
    ) -> None:
        """Async convert document with metadata."""
        mock_api.post("/api/convert").mock(
            return_value=httpx.Response(
                200,
                json={"data": {"processed": True}},
    ...
    async def test_async_convert_requires_input(
        self, async_client: AsyncClient
    ) -> None:
        """Async convert raises error when no input provided."""
        with pytest.raises(ValueError, match="Must provide one of"):
            await async_client.convert.run(document_type_code="invoice")


class TestAsyncConvertRunAsync:
    """Tests for AsyncConvert.run_async() method."""
    ...
    async def test_async_run_async_returns_status(
        ...
        status = await async_client.convert.run_async(
            url="https://example.com/large.pdf",
            document_type_code="invoice",
        )
...
class TestAsyncConvertGetStatus:
    """Tests for AsyncConvert.get_status() polling."""
    ...
    async def test_async_get_status_processing(
        ...
        status = await async_client.convert.get_status("conv_123")
